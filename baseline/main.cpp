...
void sub_task_server(int thread_index, QpHandler* handler, void* buf, void* verify_buf, size_t buf_size, int num_chunck, int chunck_size, int iterations){
...
void sub_task_client(int thread_index, QpHandler* handler, void* buf, void* verify_buf, int* compressed_lengths, size_t buf_size, int num_chunck, int chunck_size, int iterations){
	size_t ops = iterations * num_chunck;
...
void compression_benchmark(NetParam &net_param, int num_threads, int iterations, int chunck_size){
	...
	for(int i=0;i<num_threads;i++){
		if(net_param.nodeId == 0){
			threads[i] = thread(sub_task_server, i, qp_handlers[i], bufs[i], verify_buf, buf_size, num_chunck, chunck_size, iterations);
		}else if(net_param.nodeId == 1){
			threads[i] = thread(sub_task_client, i, qp_handlers[i], bufs[i], verify_buf, compressed_lengths, buf_size, num_chunck, chunck_size, iterations);
		}
		set_cpu(threads[i],i);
	}
	for(int i=0;i<num_threads;i++){
...
int main(int argc, char *argv[]){
    ...
    roce_init(net_param);
	compression_benchmark(net_param, 1, 1, 4*1024);
	...
	// int chunck_size = 4*1024;
	// int num_chunck = (file_size+chunck_size-1)/chunck_size;
